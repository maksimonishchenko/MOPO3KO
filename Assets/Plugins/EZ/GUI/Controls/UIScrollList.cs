...
public class UIScrollList : MonoBehaviour, IUIObject
{
	...
	public GameObject[] dependentItems = new GameObject[0];
	...
	public float overscrollAllowance = 0.5f; // The percentage of the viewable area that can be "over-scrolled" (scrolled beyond the end/beginning of the list)
	...
	void Start()
	{
		...
		for(int i=0; i<dependentItems.Length; ++i)
			if(dependentItems[i] != null)
			{
				IUIListObject item = (IUIListObject)dependentItems[i].GetComponent(typeof(IUIListObject));
				item.SetList(this);
			}
