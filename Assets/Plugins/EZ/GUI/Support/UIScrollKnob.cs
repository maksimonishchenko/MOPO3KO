...
public class UIScrollKnob : UIButton
{
	...
	public override void OnInput(ref POINTER_INFO ptr)
	{
		...
		switch(ptr.evt)
		{
			...
			case POINTER_INFO.INPUT_EVENT.DRAG:
				...
				newPos.y = Mathf.Clamp(newPos.y + dist,  origPos.y - maxScrollPos,origPos.y);
			
				transform.localPosition = newPos;
				//prevPoint.x = Mathf.Clamp(prevPoint.x, origPos.x - colliderExtent, origPos.x + colliderExtent + maxScrollPos);
				prevPoint.y = Mathf.Clamp(prevPoint.y, origPos.y - maxScrollPos, origPos.y);
			
				// Inform the slider that we've moved:
				slider.ScrollKnobMoved(this, GetScrollPos());
				break;
		}
		//hack make renderer on
		if (!gameObject.transform.GetChild(0).renderer.enabled)
		{
	...
	{
		return (origPos.y - transform.localPosition.y) / maxScrollPos;
	...
	public void SetPosition(float pos)
	{
		...
		transform.localPosition = origPos + (Vector3.down * maxScrollPos * pos);
	}
