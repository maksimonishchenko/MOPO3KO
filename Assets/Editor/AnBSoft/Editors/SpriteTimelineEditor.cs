	...
	*/
	...
	public bool STEOnGUI(float adjust, out float bottom)
	{
		...
		if (sprite is PackedSprite)
		{
			...
			PackedSprite p = (PackedSprite)sprite;
			
			if (p!=null && staticTexture!=null && p.gameObject.name != staticTexture.name)
			{
				//p.width = staticTexture.width;
				//p.height = staticTexture.height;
				ResizeToStaticTexture(ref p,ref staticTexture);
			}
	...
	private void ResizeToStaticTexture(ref PackedSprite p ,ref Texture2D texture)
	{
		Rect r = GetOccupiedRect(staticTexture);
				p.width  = r.width;
				p.height = r.height;
				p.gameObject.name = staticTexture.name;	
				
				if (p.collider !=null)
				{
					if (p.collider is BoxCollider)
					{
						BoxCollider c = (BoxCollider)p.collider;
						c.size = new Vector3(staticTexture.width,staticTexture.height,0);
					}
				}
	}
	...
	public void SetupSelection()
	{
		...
		if ((MonoBehaviour)sprite && sprite!= null && sprite.gameObject !=null)  
		{
	...
	public void AppendTexturesBySelection()
	{
		if (DragAndDrop.objectReferences.Length == 1 && !(DragAndDrop.objectReferences[0] is Texture2D))
		{
			...
			Texture2D[] texs = new Texture2D[filePaths.Length];
			for(int t=0;t<filePaths.Length;t++)
			{
				texs[t] = 	(Texture2D)AssetDatabase.LoadAssetAtPath(filePaths[t],typeof(Texture2D));
			}
			AppendTextures(texs);
		}
		else if (DragAndDrop.objectReferences.Length > 1 || (DragAndDrop.objectReferences.Length == 1 && (DragAndDrop.objectReferences[0] is Texture2D)))
		{
			AppendTextures(DragAndDrop.objectReferences);	
		}
	}
	
	public void AppendTextures(Object[] objs)
	{
		if (sprite.States.Length < 1)
			return;
		...
		if (sortedNewList.Count>1)
		{
			sortedNewList.Sort(new TextureNameComparer());	
		}
		
		//converted paths to Texture2D
		ArrayList alreadyInListTextures2D = new ArrayList();
		for(int i=0;i<newFrameList.Count;i++)
		{
			Texture2D tex = (Texture2D)AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(newFrameList[i] as string), typeof(Texture2D));
			alreadyInListTextures2D.Add(tex);
		}
		int cuttedDublicatedFrames = 0;
		//Debug.Log("old texutes " + alreadyInListTextures2D.Count);
		//Edited Get Rid of dublicated textures dropped onto sprite line by replace 
		for (int i = 0; i < sortedNewList.Count; i++)
		{
		//	bool cutted = false;
			
			//int index2 = alreadyInListTextures2D.BinarySearch(sortedNewList[i],new Texture2DImageComparer());
			int index2 = GetIndexOfTheTexture2D(alreadyInListTextures2D,alreadyInListTextures2D.Count,sortedNewList[i] as Texture2D);
			if (index2>=0 && index2<newFrameList.Count)
			{
				sortedNewList[i] =  alreadyInListTextures2D[index2];
				cuttedDublicatedFrames++;
				continue;
			}
			else if (objs.Length>1)
			{
				
				int index1 = GetIndexOfTheTexture2D(sortedNewList,i,sortedNewList[i] as Texture2D);
				if (index1>=0 && index1<sortedNewList.Count)
				{
					sortedNewList[i] = sortedNewList[index1];
					cuttedDublicatedFrames++;
				}
			}
		}  
		Debug.Log("Dublicated frames finded " + cuttedDublicatedFrames);
		...
		if (null==staticTexture)
		{
			Texture2D first;
			if (null!=alreadyInListTextures2D && alreadyInListTextures2D.Count>0)
			{
				first = alreadyInListTextures2D[0] as Texture2D;
			}
			else
			{
				first = sortedNewList[0] as Texture2D;
			}
			((PackedSprite)sprite).staticTexGUID = AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(first));
		}
		
		sortedNewList = null;
		alreadyInListTextures2D = null;
		newFrameList = null;
		
		
	}
	
	private int GetIndexOfTheTexture2D(ArrayList list,int indexLimit,Texture2D texture)
	{
		int ind = -1;
		for(int i =0;i<indexLimit;i++)
		{
			if (IsEqualImage(texture,list[i] as Texture2D,0f))
			{
				ind = i;
				return ind;
			}
	...
	private bool IsEqualImage(Texture2D t1,Texture2D t2,float selectivety)
	{
		...
		int selectivelyPixels = Mathf.RoundToInt(selectivety*(t1.width*t1.height));
		int errorPixels = 0;
		...
		for(int x=0;x<t1.width;x++)
		{
			...
			{
				if (t1.GetPixel(x,y) != t2.GetPixel(x,y))
				{
					errorPixels++;	
					if (errorPixels > selectivelyPixels)
					{
						return false;
					}
				}
