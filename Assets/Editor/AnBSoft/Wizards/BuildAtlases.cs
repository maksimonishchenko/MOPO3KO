...
public class BuildAtlases : ScriptableWizard
{
	...
	public UnityEngine.Object folderLowRes = null;
	public UnityEngine.Object folderHiRes = null;
	...
	void BuildAtlas(AtlasTextureList texList)
	{
		...
		if (folderHiRes != null)
		{
			string atlasHiresPath = AssetDatabase.GetAssetPath(folderHiRes) + "/" + atlas.name + ".png";
			// Write out the atlas file:
			using (FileStream fs = File.Create(atlasHiresPath))
			{
				fs.Write(bytes, 0, bytes.Length);
			}
		}
		
		if (folderLowRes != null)
		{
			string atlasLowResPath = AssetDatabase.GetAssetPath(folderLowRes) + "/" + atlas.name + ".png";
			// Write out the atlas file:
			using (FileStream fs = File.Create(atlasLowResPath))
			{
				fs.Write(bytes, 0, bytes.Length);
			}
		}
